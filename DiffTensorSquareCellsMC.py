...
def GetBorderPos(oldcoord,insidecoord,dist) :
    theta = np.arctan((insidecoord[1]-oldcoord[1])/(insidecoord[0]-insidecoord[1])) #angle between particle trajectory and x-axis
    return [oldcoord[0]-dist*np.cos(theta),oldcoord[1]-dist*np.sin(theta),oldcoord[2]]
...
def MainLoop(N_t,N_p,dt,dr,L_0,T) :
    coordinates = [np.random.uniform(-1e-4,1e-4),np.random.uniform(-1e-4,1e-4),np.random.uniform(-1e-4,1e-4)]
